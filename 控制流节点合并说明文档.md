...
flowchart LR
  ...
  subgraph R5["转换后"]
    direction TB
    E5(("新的单向节点<br/>IF语句转换<br/>Body为原单向节点"))
    F5(("单向节点"))
    E5 --> F5
  end
  L5 --> X5 --> R5
...
合并为 `while(条件)`，并将循环结构转换为新的单向节点；该新节点在循环结束后指向终止节点。
...
合并为 `while(条件)`，并将循环结构转换为新的单向节点；该新节点在循环结束后指向后继单向节点。
...
flowchart LR
  ...
  subgraph R10["转换后"]
    direction TB
    D10(("新的单向节点<br/>IF语句转换<br/>Body为原终止节点"))
    E10(("单向节点"))
    D10 --> E10
  end
  L10 --> X10 --> R10
